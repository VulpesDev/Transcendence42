...
player_1 = table.getAttribute('data-name');
table = document.getElementById("p2");
player_2 = table.getAttribute('data-name');
...
function endGame() {
  ...
  if (lang === "de") {
    let winner = leftPlayerScore >= 11 ? player_1 : player_2;
  ...
  // Sending the gamedata
  ctx.font = "20px Poppins";
  if (lang === "de")
  {
	ctx.fillText('Drücke "R", um das Resultat zu bestätigen.', pong.width / 2, pong.height / 2 + 40);
  }
  else if (lang === "it")
  {
	ctx.fillText('Premere "R" per confermare il risultato.', pong.width / 2, pong.height / 2 + 40);
  }
  else
  {
	ctx.fillText('Press "R" to confirm the result.', pong.width / 2, pong.height / 2 + 40);
  }
  ...
  document.addEventListener("keydown", function (event) {
    if (event.key === "r") {
		pong_btn.click();
		//window.location.href = "/pong_tournament_4_table";
    }
  });
}
